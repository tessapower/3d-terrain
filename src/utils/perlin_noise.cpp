...
perlin::perlin(const unsigned int seed, const unsigned int octaves,
    ...
    : m_repeat_(repeat),
      m_octaves_(octaves),
      m_lacunarity_(lacunarity),
      m_persistence_(persistence) {
  if (seed == 0) {
    // initialise permutation vector with reference values
    m_p_ = {151, 160, 137, 91,  90,  15,  131, 13,  201, 95,  96,  53,  194,
    ...
    iota(m_p_.begin(), m_p_.end(), 0);

    // shuffle with seeded random engine
    std::ranges::shuffle(m_p_.begin(), m_p_.end(),
                         std::default_random_engine(seed));
  }

  // duplicate permutation vector
  m_p_.insert(m_p_.end(), m_p_.begin(), m_p_.end());
}

float perlin::generate_perlin(const float x, const float y,
                              const float z) const {
  float total = 0.0f;
...
float perlin::noise(float x, float y, float z) const {
  ...
  const int a = m_p_[xi] + yi;
  const int aa = m_p_[a] + zi;
  const int ab = m_p_[a + 1] + zi;
  const int b = m_p_[xi + 1] + yi;
  const int ba = m_p_[b] + zi;
  const int bb = m_p_[b + 1] + zi;

  float x1 = lerp(grad(m_p_[aa], xf, yf, zf),         // a
                  grad(m_p_[ba], xf - 1.0f, yf, zf),  // b
                  u);                                 // w

  float x2 = lerp(grad(m_p_[ab], xf, yf - 1.0f, zf),         // a
                  grad(m_p_[bb], xf - 1.0f, yf - 1.0f, zf),  // b
                  u);                                        // w

  const float y1 = lerp(x1, x2, v);

  x1 = lerp(grad(m_p_[aa + 1], xf, yf, zf - 1.0f),         // a
            grad(m_p_[ba + 1], xf - 1.0f, yf, zf - 1.0f),  // b
            u);                                            // w

  x2 = lerp(grad(m_p_[ab + 1], xf, yf - 1.0f, zf - 1.0f),         // a
            grad(m_p_[bb + 1], xf - 1.0f, yf - 1.0f, zf - 1.0f),  // b
            u);                                                   // w
...
float perlin::grad(const int hash, const float x, const float y,
                   const float z) {
  switch (hash & 0xF) {
    case 0x0:
      return x + y;
