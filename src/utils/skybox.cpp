...
auto skybox::load_cube_map() const noexcept -> GLuint {
  GLuint texture_id;
...
auto skybox::load_mesh() const noexcept -> cgra::gl_mesh {
  cgra::mesh_builder mb;

  for (int i = 0; i < 36; ++i) {
    mb.push_vertex(
        {m_vertices_[i], glm::vec3{0.0f}, glm::vec3{0.0f}, glm::vec3{0.0f}});
    mb.push_index(i);
  }
...
auto skybox::draw(const glm::mat4& view,
                  const glm::mat4& projection) const noexcept -> void {
  glDepthFunc(GL_LEQUAL);
  glUseProgram(m_shader_);
  // Remove translation from the view matrix
  const auto adjusted_view = glm::mat4(glm::mat3(view));

  glUniformMatrix4fv(glGetUniformLocation(m_shader_, "uModelViewMatrix"), 1,
                     false, glm::value_ptr(adjusted_view));
  glUniformMatrix4fv(glGetUniformLocation(m_shader_, "uProjectionMatrix"), 1,
