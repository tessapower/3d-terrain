...
auto tree::draw(const glm::mat4& view, const glm::mat4& projection) const
    -> void {
...
// finds a random position within a sphere and returns a point within it
auto tree::generate_random_leaf_position(const float radius) noexcept
    -> glm::vec3 {
  // Random angle
  const float theta = glm::linearRand(0.0f, 2.0f * pi);
  // Random inclination angle
  const float phi = glm::linearRand(0.0f, pi);
...
auto tree::generate_leaves(const float radius, const int leaves) -> void {
  ...
  for (int i = 0; i < leaves; i++) {
    glm::vec3 leaf;
    leaf = process(glm::linearRand(0.0f, 1.0f)) *
  ...
  while (!found_leaf) {
    ...
    if (!found_leaf) {
      glm::vec3 random_vector = glm::normalize(
          glm::vec3(glm::linearRand(-1.0f, 1.0f), glm::linearRand(-1.0f, 1.0f),
...
auto tree::grow() -> bool {
  ...
  for (auto m_leaf_position : m_leaf_positions) {
    ...
    const auto size = static_cast<int>(m_branches.size());
  ...
  const auto branches_size = static_cast<int>(m_branches.size());
  for (auto i = branches_size - 1; i >= 0; i--) {
    ...
    if (current_branch.count > 0) {
      ...
      branch new_branch(current_branch.m_end, new_end,
                        current_branch.m_direction, i, branches_size);
      current_branch.children.push_back(branches_size);
      ...
      while (!reached_end) {
        branch_check.children.push_back(branches_size);
...
auto tree::draw_branches_nodes(int current_branch, const glm::mat4& view,
  ...
  for (auto const& b : m_branches) {
    ...
    glm::mat4 scaling_matrix = glm::scale(glm::mat4(1.0f), glm::vec3(0.3f));
...
auto tree::generate_mesh() -> void {
  ...
  float angle_increment = 2.0f * pi / static_cast<float>(num_vertices);
  ...
  for (auto j = 0; j < num_vertices; j++) {
    ...
    initial_circle.push_back(static_cast<int>(builder.m_vertices.size()));
  ...
  for (auto& b : m_branches) {
    ...
    if (b.children.empty()) {
      ...
      branch_radius = 0.0002f * static_cast<float>(b.children.size());
      branch_radius = 0.95f * (pow(branch_radius - 1.0f, 3.0f)) + 1.0f;
    }

    b.vertices.clear();
    for (auto j = 0; j < num_vertices; j++) {
      float angle = angle_increment * j;
      ...
      float angle_between_normals = glm::acos(
          glm::dot(glm::vec3(0, 1, 0), normalize(b.m_direction)));

      glm::mat4 rotation_matrix =
          glm::rotate(glm::mat4(1.0f), angle_between_normals,
                      cross(glm::vec3(0, 1, 0), normalize(b.m_direction)));
      glm::vec4 rotated_vertex = rotation_matrix * glm::vec4(x, 0.0f, z, 1.0f);

      vertex.pos = b.m_end + glm::vec3(rotated_vertex);
      vertex.norm = b.m_end + glm::vec3(rotated_vertex);
      vertex.uv = glm::vec2(0, 0);

      b.vertices.push_back(static_cast<int>(builder.m_vertices.size()));
      builder.push_vertex(vertex);
    }
  }
  ...
  for (auto r = 1; r < size; r++) {
    ...
    for (auto v = 0; v < num_vertices - 1; v++) {
...
auto tree::calculate_foliage() -> void {
  ...
  for (auto& b : m_branches) {
    if (b.children.empty()) {
      leaf new_leaf(glm::vec3(0.0f, glm::linearRand(0.4f, 0.8f), 0.0f),
                    glm::vec3(glm::linearRand(0.5f, 1.0f)), b.m_end);
